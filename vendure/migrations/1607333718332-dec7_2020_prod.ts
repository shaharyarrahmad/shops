...
export class dec72020Prod1607333718332 implements MigrationInterface {
   ...
   public async up(queryRunner: QueryRunner): Promise<any> {
        ...
        await queryRunner.query("CREATE TABLE `shipping_method_translation` (`createdAt` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `updatedAt` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `languageCode` varchar(255) NOT NULL, `name` varchar(255) NOT NULL DEFAULT '', `description` varchar(255) NOT NULL DEFAULT '', `id` int NOT NULL AUTO_INCREMENT, `baseId` int NULL, PRIMARY KEY (`id`)) ENGINE=InnoDB", undefined);
        ...
        await queryRunner.query("ALTER TABLE `stock_movement` ADD `orderLineId` int NULL", undefined);
        await queryRunner.query("ALTER TABLE `product_variant` ADD `stockAllocated` int NOT NULL DEFAULT 0", undefined);
        await queryRunner.query("ALTER TABLE `product_variant` ADD `outOfStockThreshold` int NOT NULL DEFAULT 0", undefined);
        await queryRunner.query("ALTER TABLE `product_variant` ADD `useGlobalOutOfStockThreshold` tinyint NOT NULL DEFAULT 1", undefined);
        await queryRunner.query("ALTER TABLE `global_settings` ADD `outOfStockThreshold` int NOT NULL DEFAULT 0", undefined);
        await queryRunner.query("ALTER TABLE `product_variant` DROP COLUMN `trackInventory`", undefined);
        await queryRunner.query("ALTER TABLE `product_variant` ADD `trackInventory` varchar(255) NOT NULL DEFAULT 'INHERIT'", undefined);
        await queryRunner.query("ALTER TABLE `global_settings` CHANGE `trackInventory` `trackInventory` tinyint NOT NULL DEFAULT 1", undefined);
   ...
   public async down(queryRunner: QueryRunner): Promise<any> {
        ...
        await queryRunner.query("ALTER TABLE `global_settings` CHANGE `trackInventory` `trackInventory` tinyint NOT NULL DEFAULT '0'", undefined);
        await queryRunner.query("ALTER TABLE `product_variant` DROP COLUMN `trackInventory`", undefined);
        await queryRunner.query("ALTER TABLE `product_variant` ADD `trackInventory` tinyint NOT NULL", undefined);
        ...
        await queryRunner.query("ALTER TABLE `shipping_method` ADD `description` varchar(255) NOT NULL", undefined);
        await queryRunner.query("ALTER TABLE `order_item` ADD `unitPriceIncludesTax` tinyint NOT NULL", undefined);
        await queryRunner.query("DROP TABLE `shipping_method_translation`", undefined);
        await queryRunner.query("CREATE UNIQUE INDEX `REL_cbb0990e398bf7713aebdd3848` ON `stock_movement` (`orderItemId`)", undefined);
        await queryRunner.query("ALTER TABLE `history_entry` ADD CONSTRAINT `FK_43ac602f839847fdb91101f30ec` FOREIGN KEY (`customerId`, `customerId`) REFERENCES `customer`(`id`,`id`) ON DELETE NO ACTION ON UPDATE NO ACTION", undefined);
        await queryRunner.query("ALTER TABLE `history_entry` ADD CONSTRAINT `FK_3a05127e67435b4d2332ded7c9e` FOREIGN KEY (`orderId`, `orderId`) REFERENCES `order`(`id`,`id`) ON DELETE NO ACTION ON UPDATE NO ACTION", undefined);
   }
