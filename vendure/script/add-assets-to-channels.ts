...
export async function addToDefaultChannel(queryRunner: QueryRunner, tableName: string, idName: string, channelCode: string) {
    ...
    const result = await queryRunner.query(
        ...
        '` (' +
        idName +
        ", channelId) SELECT id, (SELECT id from `channel` WHERE `code` = '" + channelCode + "') FROM `" +
        tableName +
        '`',
        [],
    );
