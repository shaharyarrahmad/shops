import {
  ShippingTaxSummaryLine,
  TaxSummary,
  ProductTaxSummaryLine,
  TaxTotal,
} from './tax-summary';
...
export class TaxHelper {
  ...
  static getTaxSummary(order: Order): TaxSummary {
    ...
    let totalVAT = order.taxSummary
      .map((summary) => summary.taxTotal)
      .reduce((a, b) => a + b); // total taxes without shippingTax
    // Calculate shipping
    const maxTax = Math.max(
      ...order.taxSummary.map((summary) => summary.taxRate)
    );
