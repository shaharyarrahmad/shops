...
import { Administrator, Logger, OrderStateTransitionEvent, TransactionalConnection } from '@vendure/core';
...
export const orderConfirmationHandler: EmailEventHandler<any, any> = new EmailEventListener('order-confirmation')
  ...
  .loadData(async ({ event, injector }) => {
    ...
    let admins: AdminWithChannel[] = await injector.get(TransactionalConnection).getRepository(Administrator).createQueryBuilder('admin')
      .innerJoin('admin.user', 'user')
      .innerJoin('user.roles', 'role')
      .innerJoinAndSelect('role.channels', 'channel', 'channel.id = :channelId', { channelId: channel.id })
      .execute();
    ...
    if (!admins?.[0]?.admin_emailAddress) {
      Logger.error(`No admin found to send confirmation email for channel with id ${channel.id}`);
    }
    const adminRecipients = admins.map(admin => admin.admin_emailAddress);
    Logger.info(`Sending order confirmation email to ${adminRecipients} for channel ${channelName}`);
    return { channelName, adminRecipients };
  })
  .setRecipient(
    (event) =>
      `${event.order.customer!.emailAddress},${event.data.adminRecipients.join(',')}`
  )
  .setFrom(`{{ fromAddress }}`)
  .setSubject(
