    ...
    >
      ...
      <div class="cell product-details">
        ...
        <select
          ...
          v-on:change="selectVariant($event.target.value)"
          aria-label="Selectable editions"
        >
          <option
            v-for="variant of $context.product.variants"
            :value="variant.id"
...
export default {
  ...
  methods: {
    ...
    selectVariant(variantId) {
      const newVariant = this.$context.product.variants.find(
        (v) => v.id === variantId
      );
      if (!newVariant) {
        return;
    ...
    load() {
      ...
      const variant =
        this.$context.product.variants.find(
          (v) => v.stockLevel !== 'OUT_OF_STOCK'
        ) || this.$context.product.variants[0];
      this.selectVariant(variant.id);
      this.asset =
        this.selectedVariant.featuredAsset ||
        this.getDefaultAsset(this.$context.product);
      this.assets = this.getAssets(this.selectedVariant);
    },
  },
  created() {
  ...
  async mounted() {
    this.$context.product = await this.$vendure.getProduct(
      this.$context.product.slug
    );
    this.load();
  },
};
