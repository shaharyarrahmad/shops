...
export async function proceedToArrangingPayment(shopClient: SimpleGraphQLClient): Promise<Order> {
    ...
    const { transitionOrderToState } = await shopClient.query(TRANSITION_TO_STATE, { state: 'ArrangingPayment' });
    return transitionOrderToState;
}

export async function addPaymentToOrder(shopClient: SimpleGraphQLClient, handler: PaymentMethodHandler): Promise<Order> {
...
export async function settlePayment(shopClient: SimpleGraphQLClient): Promise<Order> {
